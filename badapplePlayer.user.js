// ==UserScript==
// @name         WebManipulator - Bad Apple Player
...
// 📌 버튼 클릭 시 모든 이미지를 '재생되는 비디오'로 변경하는 기능
aimageChangeButton.addEventListener("click", () => {
    // 1. 파일 선택창을 만듭니다. 이제 이미지뿐만 아니라 비디오도 선택할 수 있습니다.
    const fileInput = document.createElement('input');
    fileInput.type = "file";
    fileInput.accept = "video/*,image/*"; // 비디오와 이미지를 모두 허용

    // 2. 파일이 선택되었을 때의 동작을 정의합니다.
    fileInput.addEventListener("change", function () {
        const file = fileInput.files[0];
        if (!file) return;

        // 비디오 파일인 경우에만 특별한 처리를 합니다.
        if (file.type.startsWith('video/')) {
            // 3. FileReader 대신 URL.createObjectURL을 사용해 비디오의 임시 URL을 생성합니다.
            //    이 URL은 브라우저에서 직접 재생할 수 있습니다.
            const videoURL = URL.createObjectURL(file);

            // 4. 페이지의 모든 <img> 태그를 찾아서 반복 처리합니다.
            document.querySelectorAll("img").forEach(img => {
                // 5. 각 이미지 태그 위치에 새로 <video> 태그를 만듭니다.
                const newVideo = document.createElement('video');
                newVideo.src = videoURL;         // 비디오 소스를 방금 만든 URL로 설정
                newVideo.autoplay = true;        // 자동 재생
                newVideo.loop = true;            // 반복 재생
                newVideo.muted = false;           // 자동 재생을 위해 음소거는 필수입니다.
                newVideo.style.objectFit = 'cover'; // 이미지가 채워지던 방식과 유사하게 설정

                // 6. 기존 이미지의 크기와 스타일을 최대한 따라가도록 설정합니다.
                //    이렇게 해야 페이지 레이아웃이 깨지는 것을 최소화할 수 있습니다.
                newVideo.style.width = `${img.width}px`;
                newVideo.style.height = `${img.height}px`;
                newVideo.className = img.className; // 기존 클래스를 그대로 적용

                // 7. 준비된 비디오로 기존 이미지를 교체합니다.
                img.replaceWith(newVideo);
            });
            console.log("✅ 모든 이미지가 비디오로 교체되었습니다.");

        } else { // 이미지 파일인 경우 기존 방식대로 처리
            const reader = new FileReader();
            reader.onload = function (e) {
                document.querySelectorAll("img").forEach(img => {
                    img.src = e.target.result;
                });
                console.log("✅ 모든 이미지가 선택한 이미지로 변경되었습니다.");
            };
...
// 📌 Video Wall Mode 버튼 생성
const videoWallButton = document.createElement('button');
videoWallButton.innerText = "Video Wall Mode";
videoWallButton.style.width = "100%";
videoWallButton.style.padding = "8px";
videoWallButton.style.marginBottom = "8px";
videoWallButton.style.backgroundColor = "#000";
videoWallButton.style.color = "#fff";
videoWallButton.style.border = "none";
videoWallButton.style.borderRadius = "4px";
videoWallButton.style.cursor = "pointer";
panel.appendChild(videoWallButton);

// 📌 Video Wall Mode 기능 로직
videoWallButton.addEventListener("click", () => {
    const fileInput = document.createElement('input');
    fileInput.type = "file";
    ...
    fileInput.addEventListener("change", function () {
        ...
        // 1. 숨겨진 비디오 엘리먼트를 생성하고 페이지에 추가합니다.
        const video = document.createElement('video');
        video.src = URL.createObjectURL(file);
        video.muted = false; // 자동 재생 정책을 위해 음소거는 필수입니다.
        video.loop = true;
        video.playsInline = true; // 모바일 환경에서의 재생을 돕습니다.
        video.style.display = 'none';
        document.body.appendChild(video);

        // 2. 비디오가 재생 준비가 되면(loadeddata), 명시적으로 재생을 시작시킵니다.
        video.addEventListener('loadeddata', () => {
            // video.play()는 Promise를 반환합니다. 성공적으로 재생이 시작된 후에 다음 작업을 진행합니다.
            video.play().then(() => {
                // ================== [디버깅 핵심] ==================
                // 비디오가 '실제로 재생을 시작한 후'에 캔버스 교체 및 그리기 작업을 시작합니다.
                setupVideoWall(video);
            }).catch(error => {
                console.error("비디오 자동 재생에 실패했습니다. 사용자가 페이지와 상호작용해야 할 수 있습니다.", error);
                alert("비디오 재생에 실패했습니다. 페이지를 클릭한 후 다시 시도해 주세요.");
            });
        }, { once: true });
    });
...
// 비디오 월을 설정하고 그리는 모든 로직을 이 함수 안에 캡슐화합니다.
// 비디오 월을 설정하고 그리는 모든 로직을 이 함수 안에 캡슐화합니다.
function setupVideoWall(video) {
    const allImages = Array.from(document.querySelectorAll("img")).filter(img => {
        const rect = img.getBoundingClientRect();
        return rect.width > 20 && rect.height > 20 && rect.width < window.innerWidth;
    });

    if (allImages.length === 0) {
        alert("분할 재생에 적합한 이미지를 찾을 수 없습니다.");
        return;
    }
    ...
    allImages.forEach(img => {
        ...
        // ### ⬇️ 코드 수정 부분 ⬇️ ###
        // 원본 영상의 비율을 유지하도록 소스 영역을 다시 계산합니다.

        const sourceX = ((rect.left + window.scrollX - minX) / totalWidth) * video.videoWidth;
        const sourceY = ((rect.top + window.scrollY - minY) / totalHeight) * video.videoHeight;

        let sourceWidth = (rect.width / totalWidth) * video.videoWidth;
        let sourceHeight = (rect.height / totalHeight) * video.videoHeight;
        ...
        if (canvasAspectRatio > videoAspectRatio) {
            const newSourceHeight = sourceWidth / canvasAspectRatio;
            // 세로를 기준으로 비디오를 더 많이 가져와서 채웁니다.
            sourceHeight = newSourceHeight;
        }
        // 캔버스가 비디오보다 '세로로 길면'
        else if (canvasAspectRatio < videoAspectRatio) {
            const newSourceWidth = sourceHeight * canvasAspectRatio;
            // 가로를 기준으로 비디오를 더 많이 가져와서 채웁니다.
            sourceWidth = newSourceWidth;
        }
        // 비율이 같으면 그대로 둡니다.
        // ### ⬆️ 코드 수정 완료 ⬆️ ###

        allCanvases.push({ canvas, ctx, sourceX, sourceY, sourceWidth, sourceHeight });
    });

    // 애니메이션 루프 함수
    function renderAllCanvases() {
        if (video.paused || video.ended || allCanvases.length === 0) return;

        allCanvases.forEach(item => {
            item.ctx.drawImage(
                video,
    ...
    console.log(`✅ ${allImages.length}개의 이미지가 하나의 비디오 월로 구성되었습니다.`);
}
